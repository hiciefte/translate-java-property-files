...
if [ "$(id -u)" -eq 0 ]; then
    ...
    # Guarded chown for the target repository directory
    ...
    if ! [[ "$LOG_DIR_MODE" =~ ^0?[0-7]{3,4}$ ]]; then
        log "Warning: invalid LOG_DIR_MODE '$LOG_DIR_MODE' provided; defaulting to 0755"
        LOG_DIR_MODE="0755"
    fi
    mkdir -p /app/logs
    if find /app/logs -mindepth 1 -maxdepth 1 ! -user appuser -o ! -group appuser | read -r; then
...
if [ -d "$TARGET_REPO_DIR/.git" ]; then
    ...
    if [ -z "${FORK_REPO_NAME:-}" ] || [ -z "${UPSTREAM_REPO_NAME:-}" ]; then
        log "Error: FORK_REPO_NAME and UPSTREAM_REPO_NAME must be set for initial clone."
        exit 1
    fi
...
# If 'upstream' is missing, try to add it from UPSTREAM_REPO_NAME.
# This is crucial for the fork-based workflow.
if ! git remote | grep -q "^upstream$"; then
    if [ -n "${UPSTREAM_REPO_NAME:-}" ]; then
        ...
        REMOTE="upstream" # Prefer upstream if we just added it.
    elif [ -z "$REMOTE" ]; then
        log "Error: No 'upstream' or 'origin' remote found, and UPSTREAM_REPO_NAME is not set."
        exit 1
    fi
fi
# If we still don't have a remote selected (e.g., only 'origin' exists), default to it.
if [ -z "$REMOTE" ]; then
    REMOTE="origin"
fi
...
if [ "$#" -eq 0 ]; then
  log "Error: no command provided to exec. Set CMD in Dockerfile or command in docker-compose.yml."; exit 1
fi
